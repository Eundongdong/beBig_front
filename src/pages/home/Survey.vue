...
const submitSurvey = () => {
  ...
  const finType1Count2 = answers.value.filter(
    (a, i) => shuffledQuestions[i].finTestType === 1 && a === 2
  ).length;

  // finTestType이 2일 때의 답변 개수를 카운트
  const finType2Count1 = answers.value.filter(
    (a, i) => shuffledQuestions[i].finTestType === 2 && a === 1
  ).length;
  const finType2Count2 = answers.value.filter(
    (a, i) => shuffledQuestions[i].finTestType === 2 && a === 2
  ).length;
