<template>
  ...
  const formData = ref({
    ...
    gender: route.query.gender === 'true', 
    nickname: '',
    userLoginType: route.query.userLoginType || 'kakao'
  });
  ...
  const submitSignup = async () => {
    console.log("FormData before submission:", formData.value);
    try {
      const response = await UserApi.socialSignupRegister(formData.value);
      if (response) {
        alert('가입이 완료되었습니다!');
        router.push('/home');
      }
  ...
  onMounted(() => {
    console.log("Route query data:", route.query);
    loadSocialLoginData();
    getTerms();
  });
